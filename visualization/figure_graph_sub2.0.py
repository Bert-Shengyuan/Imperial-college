...
def calculate_average_path_length(graph):
    ...
    if len(finite_paths) > 0:
        ...
        return average_path_length, local_effiency
    else:
        return 0, 0
...
def build_graph(g, Community):
    ...
    Wi = np.where(Wi >= 0.05, 1, 0)
    avg_path_length_W,local_efficiency_W = calculate_average_path_length(Wi)
    ...
    Be = np.where(Be >= 0.05, 1, 0)
    avg_path_length_B,local_efficiency_B = calculate_average_path_length(Be)
...
for index in range(len(dy_list_fam1)):  #(9,10)# len(dy_list)
    ...
    if gene_list_young[index] == 119:
        ...
        dy_list_df = pd.DataFrame(dy_matrix)

        # 根据排序后的community重新排列神经元

        for community in Community:
            subgraph = dy_list_df[labels_df['community'] == community]
